/**
 * Catálogo de APIs del agente (fuente única).
 *
 * Extraído de `seedApis.mjs` para que el seed idempotente (`seed.mjs`) pueda
 * reutilizarlo sin ejecutar el `main()` destructivo de aquel script.
 *
 * FR-010 de la spec 001: el baseUrl por defecto sale de OLLAMA_BASEURL; el literal
 * solo actúa como último recurso.
 */
