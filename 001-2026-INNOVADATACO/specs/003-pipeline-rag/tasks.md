...
**Estado**: ✅ **IMPLEMENTADO SIN TURNO (2026-07-23): T001–T034, T036–T040.** Detenido en la
frontera del trabajo pesado. **T035 (fe de erratas §3.4) queda para el CIERRE** —la spec no
cierra hasta que pasen TP-1…TP-4—; **T041–T044 exigen turno de Jelkin** y no se cruzaron.
...
- [x] T001 Reconfirmar baseline (quickstart §0) sobre el código del día (§1.1): `DocumentoChunk`
...
- [x] T002 [P] Confirmar rama `feature/001-scaffolding` y que el stack responde en 5001.
      **No bajarlo.**
- [x] T003 Migración `add_rag_pipeline`: añade `embeddingModel` y `enrichConfig` al modelo
...
- [x] T004 **Gate esquema** sobre BD **desechable** (no la del CEO, D-039):
...
- [x] T005 [P] [US1] `src/lib/chunker.ts`: troceado **estructural** (CONSIDERANDO/RESUELVE/
...
- [x] T006 [P] [US1] `src/lib/chunker.test.ts`: 1800 respetado; solape 200; texto corto → 1
      fragmento; vacío/espacios → 0; estructura normativa cortada por sus marcas; sin partir
      palabras. → FR-001, FR-002, §0.2
- [x] T007 [P] [US1] `src/lib/enrich.ts`: `construirPrefijo(doc, config)` y
...
- [x] T008 [P] [US1] `src/lib/enrich.test.ts`: default → texto idéntico al contenido (SC-021);
...
- [x] T009 [US2] `src/lib/ragConfig.ts`: resuelve `embedding_model` desde `ModuleSetting`
...
- [x] T010 [US2] `src/lib/ragConfig.test.ts`: precedencia en los tres casos; ausencia de
      `embedding_model` → error, no adivina; parámetros leídos de config, no literales. → FR-006, SC-014
- [x] T011 **Gate piezas puras**: `npx vitest run src/lib` verde; sin red ni BD.
...
- [x] T012 [US2] `src/lib/modelClients.ts`: `embedText(model, texto)` que llama a
...
- [x] T013 [US2] Extender `src/lib/modelClients.test.ts`: `embedText` con `fetch` mockeado
...
- [x] T014 [US3] `scripts/worker.mjs`: tras el análisis actual, paso de vectorización —
...
- [x] T015 [US3] Reintentos ante fallo transitorio de embeddings (política pg-boss existente);
...
- [x] T016 [US3] Auditoría del paso de vectorización: fragmentos generados, modelo usado,
      latencia, error si lo hubo (FR-025). → FR-025, SC-018
- [x] T017 [US3] Arnés de prueba del job con embeddings **mockeados** (`scripts/worker.test.mjs`
...
- [x] T018 **Gate ingesta**: pruebas del job verdes sin BD ni Ollama.
...
- [x] T019 [US4] `src/lib/search/filtros.ts`: construye el `WHERE` de metadatos (`tipo`,
      `entidad`, `sector`, fechas) + `activo = true`, **parametrizado**, aplicado antes de las
      ramas. → FR-014, FR-022, SC-015
- [x] T020 [US4] `src/lib/search/rrf.ts`: fusión **RRF** de dos rankings con pesos y top-k
      **de configuración**; puro y testeable. → FR-015, FR-024
- [x] T021 [US4] `src/lib/search/hibrida.ts`: consulta `$queryRaw` **parametrizada** (FR-012)
...
- [x] T022 [US4] `src/app/api/documents/search/route.ts`: sustituir la puntuación en memoria
...
- [x] T023 [P] [US4] `src/lib/search/rrf.test.ts`: distintos pesos → distinto orden; top-k
      recorta; empates estables. → FR-024, SC-014
- [x] T024 [US4] `src/app/api/documents/search/route.test.ts` extendido: con ramas y embedding
...
- [x] T025 [US4] **Gate búsqueda**: `grep -c "text.includes" …/search/route.ts` → **0**;
...
- [x] T026 [US5] `scripts/backfill-embeddings.mjs`: recorre documentos con texto y **sin**
...
- [x] T027 [US5] Contador de **pendientes de re-vectorizar** que cubre **las dos causas**
      (cambio de modelo y de enriquecimiento), consultable. → FR-021c, SC-017
- [x] T028 [US5] Prueba del backfill con embeddings mockeados: procesa solo los que faltan;
      segunda ejecución no cambia el total; reanuda sin repetir. → FR-016, SC-007
- [x] T029 **Gate backfill (código)**: pruebas verdes sin infraestructura. La **ejecución** es
      T043 (TP-3, turno).
...
- [x] T030 [US6] Verificar que **todos** los parámetros del RAG salen de configuración
      ...
      → FR-024, SC-020
- [x] T031 [US6] Métricas de búsqueda y vectorización consultables desde auditoría (latencia,
      modelo, fragmentos, si hubo evidencia). → FR-025, SC-018
- [x] T032 [US6] **Gate suite**: `npx vitest run` ≥ 181 verde sin BD ni Ollama; ninguna prueba
...
- [x] T033 [US7] Corregir la **fuga de etiqueta** del banco (FR-028, D-032): en
...
- [x] T036 Aislamiento: puertos 5005/5433/5010/5434 sin cambios; `git diff --cached
      --name-only` solo rutas de `001-2026-INNOVADATACO/`. → FR-020, SC-012
- [x] T037 Revisión de contratos spec 002/005: cero `any` nuevos, cero fugas de `err.message`,
      `verifyAuth` intacto en `search`. → FR-018, SC-009
- [x] T038 Verificar `slice(0,3000)` de `documentProcessor.ts` **intacto** (FR-003). → FR-003
- [x] T039 Actualizar `research.md` con lo que quede medido (si se barre el solape) o dejar
      constancia explícita de que sigue sin medirse. → D-029
- [x] T040 Reporte de una línea a ZEUS; commits scopeados y pusheados.
...
## Resultado sin turno (2026-07-23)

Implementado y verificado con embeddings mockeados, sin BD ni Ollama. Commits, uno por bloque:
`ed2cdec7` migración · `64913aa5` chunker/enrich/ragConfig · `315bac3f` embedText ·
`7219af17` ingesta · `29fea822` búsqueda híbrida · `c994a451` backfill · `24461664` banco.

| Gate | Resultado |
|---|---|
| Suite sin BD ni Ollama | **249 verdes** en 36 archivos (baseline 181) |
| `npx tsc --noEmit` | limpio |
| `npx eslint src/lib src/app/api` | **0** `no-explicit-any` |
| Migración (HNSW coseno + GIN + CASCADE, btree fuera) | **verificada en BD desechable** (D-039): FTS sin tildes, coseno por HNSW y CASCADE comprobados; BD desechable eliminada |
| `text.includes` en `documents/search` | **0** (ranking en Postgres, FR-015b) |
| `slice(0,3000)` de `documentProcessor` | **intacto** (FR-003) |
| Puertos 5005/5433/5010/5434 | sin cambios |
| Banco: fuga de etiqueta (SC-024) | corregida y verificada por `questions.test.mjs` |

**Cobertura con mocks**: chunker (FR-001/002), enrich (FR-026/027/028), ragConfig
(FR-004/006/023/024), embedText (FR-003/007), ingesta (FR-008/009/010/021), búsqueda híbrida
+ RRF + filtros (FR-011…015/021b), backfill + contador de pendientes (FR-016/021c).

## Frontera del trabajo pesado — DETENIDO AQUÍ

- **T035 (§3.4)**: pendiente **al cierre**. Reescribir §3.4 ahora afirmaría que el pipeline
  RAG funciona, y eso no está probado con inferencia real: sería el mismo tipo de afirmación
  falsa que originó H-05. Se hace cuando TP-1…TP-4 lo validen.
- **T041–T044 (TP-1…TP-4)**: **exigen turno de Jelkin** (ADR_002). No cruzados.
- **Despliegue en el stack vivo del CEO NO realizado**: aplicar la migración a la BD viva y
