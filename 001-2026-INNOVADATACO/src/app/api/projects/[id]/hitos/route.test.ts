...
const url = "http://localhost:5001/api/projects/p1/hitos";
...
beforeEach(async () => {
  ...
  vi.mocked(prisma.hitoProyecto.findMany).mockReset();
  vi.mocked(prisma.hitoProyecto.create).mockReset();
});

describe("GET /api/projects/[id]/hitos (spec 008, US4)", () => {
  it("rechaza con 401 sin sesión, sin tocar la base", async () => {
    ...
    expect(prisma.hitoProyecto.findMany).not.toHaveBeenCalled();
  });
  ...
  it("lista solo lo del proyecto de la ruta", async () => {
    ...
    vi.mocked(prisma.hitoProyecto.findMany).mockResolvedValue([] as never);

    const res = await GET(new NextRequest(url), params);

    expect(res.status).toBe(200);
    expect(vi.mocked(prisma.hitoProyecto.findMany).mock.calls[0][0]?.where).toEqual({
      proyectoId: "p1",
    });
  });
});

describe("POST /api/projects/[id]/hitos (spec 008, US4)", () => {
  it("rechaza con 401 sin sesión y no crea nada", async () => {
    await sinSesion();

    const res = await POST(peticionJson(url, { nombre: "Kickoff", fecha: "2026-09-01" }), params);

    expect(res.status).toBe(401);
    expect(prisma.hitoProyecto.create).not.toHaveBeenCalled();
  });

  it("crea y responde 201", async () => {
    vi.mocked(prisma.proyecto.findUnique).mockResolvedValue(PROYECTO as never);
    vi.mocked(prisma.hitoProyecto.create).mockResolvedValue({ id: "x1" } as never);

    const res = await POST(peticionJson(url, { nombre: "Kickoff", fecha: "2026-09-01" }), params);

    expect(res.status).toBe(201);
    expect(vi.mocked(prisma.hitoProyecto.create).mock.calls[0][0].data).toMatchObject({
      proyectoId: "p1",
    });
  });
  ...
  it("rechaza con 400 lo inválido, con mensaje legible", async () => {
    ...
    const res = await POST(peticionJson(url, { nombre: "Sin fecha" }), params);
