...
const url = "http://localhost:5001/api/projects/p1/recursos";
...
beforeEach(async () => {
  ...
  vi.mocked(prisma.recursoProyecto.findMany).mockReset();
  vi.mocked(prisma.recursoProyecto.create).mockReset();
});

describe("GET /api/projects/[id]/recursos (spec 008, US5)", () => {
  it("rechaza con 401 sin sesión, sin tocar la base", async () => {
    ...
    expect(prisma.recursoProyecto.findMany).not.toHaveBeenCalled();
  });
  ...
  it("lista solo lo del proyecto de la ruta", async () => {
    ...
    vi.mocked(prisma.recursoProyecto.findMany).mockResolvedValue([] as never);

    const res = await GET(new NextRequest(url), params);

    expect(res.status).toBe(200);
    expect(vi.mocked(prisma.recursoProyecto.findMany).mock.calls[0][0]?.where).toEqual({
      proyectoId: "p1",
    });
  });
});

describe("POST /api/projects/[id]/recursos (spec 008, US5)", () => {
  it("rechaza con 401 sin sesión y no crea nada", async () => {
    await sinSesion();

    const res = await POST(peticionJson(url, { nombre: "Ana", rol: "Líder", tipo: "humano", costo: 100 }), params);

    expect(res.status).toBe(401);
    expect(prisma.recursoProyecto.create).not.toHaveBeenCalled();
  });

  it("crea y responde 201", async () => {
    vi.mocked(prisma.proyecto.findUnique).mockResolvedValue(PROYECTO as never);
    vi.mocked(prisma.recursoProyecto.create).mockResolvedValue({ id: "x1" } as never);

    const res = await POST(peticionJson(url, { nombre: "Ana", rol: "Líder", tipo: "humano", costo: 100 }), params);

    expect(res.status).toBe(201);
    expect(vi.mocked(prisma.recursoProyecto.create).mock.calls[0][0].data).toMatchObject({
      proyectoId: "p1",
    });
  });
  ...
  it("rechaza con 400 lo inválido, con mensaje legible", async () => {
    ...
    const res = await POST(peticionJson(url, { nombre: "X", tipo: "extraterrestre" }), params);
