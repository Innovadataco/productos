...
echo "==> [3/4] Levantando app (:5005, -H 0.0.0.0) + UN worker + UN monitor + UN simulador-abuso"
...
echo "Procesos:"; ps aux | grep -E "next start|worker-reportes|monitor-probes|simulador-abuso" | grep -v grep || true
echo "Logs: tail -f /tmp/app-002.log | tail -f /tmp/worker-002.log | tail -f /tmp/monitor-002.log | tail -f /tmp/simulador-abuso-002.log"
