...
echo "==> [3/4] Levantando app (:5005, -H 0.0.0.0) + UN worker + UN monitor"
...
echo "Procesos:"; ps aux | grep -E "next start|worker-reportes|monitor-probes" | grep -v grep || true
echo "Logs: tail -f /tmp/app-002.log | tail -f /tmp/worker-002.log | tail -f /tmp/monitor-002.log"
