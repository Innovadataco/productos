#!/usr/bin/env tsx
/**
 * Eval harness para F0.5 (taxonomía ampliada + nuevas categorías).
 * Uso:
 *   node --env-file=.env --import tsx scripts/eval-classifier-f0_5.ts [fixture.json]
 */
...
async function main() {
    ...
    for (let i = 0; i < examples.length; i++) {
        ...
        try {
            ...
            console.log(`${correct ? "OK" : "FAIL"} | ${ex.expected} -> ${res.categoria} (${res.estado}) ${latencyMs}ms`);
        } catch (err) {
            const latencyMs = Date.now() - start;
            results.push({
                text: ex.text,
    ...
    const outFile = path.join(outDir, `f0_5-${Date.now()}.json`);
