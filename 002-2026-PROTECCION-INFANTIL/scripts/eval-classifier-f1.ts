#!/usr/bin/env tsx
/**
 * Eval harness para F1 (structured outputs nativos).
 * Uso:
 *   node --env-file=.env --import tsx scripts/eval-classifier-f1.ts [fixture.json]
 */
...
async function main() {
    ...
    const outFile = path.join(outDir, `f1-${Date.now()}.json`);
