...
- [x] T001 [US1] Generate DB dump before any migration/seed change (`scripts/backup-pre-073.sh` or manual `pg_dump`).
- [x] T002 [US1] Create additive Prisma migration `add_departamento`:
...
- [x] T003 [US1] Update `prisma/schema.prisma`:
  - Add `model Departamento`.
  - Add `departamentoId String?` to `Ciudad`.
  - Add `departamentos` relation to `Pais`.
- [x] T004 [US1] Verify migration applies with `npx prisma migrate deploy` on a copy of the DB (no data loss).
...
- [x] T005 [P] [US1] Integration test: `prisma/departamento.test.ts` (or equivalent) — verify `Departamento` CRUD and `Ciudad.departamentoId` nullable behavior.
- [x] T006 [P] [US1] Migration test: verify existing `Ciudad` rows still have `paisId` and nullable `departamentoId` after migration.

### Implementation for User Story 1

- [x] T007 [US1] Write `Departamento` model in `schema.prisma` (already in T003).
- [x] T008 [US1] Generate migration with `npx prisma migrate dev --name add_departamento` (after plan approval).
...
- [x] T009 [P] [US2] Integration test: after seed, Colombia has 33 departments.
- [x] T010 [P] [US2] Integration test: existing 10 Colombian cities have `departamentoId` not null.
- [x] T011 [P] [US2] Idempotency test: running seed twice does not duplicate departments or cities.
- [x] T012 [P] [US2] City-to-department mapping test: verify each existing city is linked to the correct department (Bogotá → Cundinamarca/Bogotá D.C., Medellín → Antioquia, etc.).

### Implementation for User Story 2

- [x] T013 [US2] Add Colombia department data to `prisma/seed.ts` or `scripts/seed-colombia.ts`.
- [x] T014 [US2] Add city mapping for the 10 existing Colombian cities and principal capitals.
- [x] T015 [US2] Implement upsert logic: country by `codigo`, department by `(nombre, paisId)`, city by `(nombre, paisId)` with `departamentoId` update.
- [x] T016 [US2] Verify coordinates remain intact for existing cities.
...
- [x] T017 [P] [US3] Run full `npm run test` and confirm ≥ 600 tests pass with no modifications.
- [x] T018 [P] [US3] API test: `/api/ciudades?paisId=CO` returns same shape and cities as before.
- [x] T019 [P] [US3] API test: create a report with Colombia + Bogotá; verify `pais`, `ciudad`, `paisId`, `ciudadId` stored correctly and `departamentoId` not required.
- [x] T020 [P] [US3] Smoke test: public dashboard and admin dashboard still render city-based aggregations.

### Implementation for User Story 3

- [x] T021 [US3] Confirm no changes to `src/components/modules/ReporteStepUbicacion.tsx`, `ReporteStepDetalle.tsx`, or `ReporteWizard.tsx`.
- [x] T022 [US3] Confirm no changes to `/api/paises` and `/api/ciudades` endpoints.
- [x] T023 [US3] Confirm no changes to `crearReporteSchema` or report creation routes.
...
- [x] T024 [P] Run `quickstart.md` steps end-to-end on a fresh DB copy.
- [x] T025 [P] Verify `npx tsc --noEmit` passes.
- [x] T026 [P] Verify `npm run lint` passes.
- [x] T027 [P] Verify `npm run test` passes.
- [x] T028 [P] Verify `npm run build` compiles successfully.
- [x] T029 [P] Update `spec.md` with Implementation section and Status `CERRADA`.
- [x] T030 [P] Create `cierre.md` with evidence (git log, files touched, test results, deploy status).
- [x] T031 [P] Deploy clean with `./scripts/dev-restart.sh` and verify healthcheck.
