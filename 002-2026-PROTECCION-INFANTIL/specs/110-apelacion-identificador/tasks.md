...
- [x] T001 Schema Prisma: enum `EstadoApelacion`, modelos `Apelacion`,
...
- [x] T002 Migración aditiva con índice único parcial de apelación abierta
  (`prisma/migrations/<ts>_apelacion_identificador/migration.sql`); aplicar a dev y test.
- [x] T003 Seed de los 4 parámetros `apelacion.*` (`prisma/seed.ts`); reset de las 3
  tablas nuevas en `src/lib/test-utils.ts`.

## Fase 2 — Librerías de dominio

- [x] T004 `src/lib/apelaciones.ts`: parámetros con fallback (15/10/30/5),
...
- [x] T005 `src/lib/apelacion-storage.ts`: cifrar/descifrar buffer AES-256-GCM
...
- [x] T006 Visibilidad: `ocultoPorComiteEn` en `src/lib/visibility.ts`; levantar marca en
  el upsert de reporte nuevo de `src/app/api/reportes/route.ts`. `.gitignore` += `storage/`.

## Fase 3 — APIs del apelante

- [x] T007 `POST /api/apelaciones` (multipart: campos + PDF; validaciones Zod, PDF,
...
- [x] T008 `GET /api/apelaciones/mias` (solo propias; N reportes; decisión + motivación;
  SIN datos de reportes) + test (sin fuga de contenido).

## Fase 4 — APIs del comité

- [x] T009 `GET /api/admin/comite/apelaciones` (bandeja: filtro estado, marca próximo a
...
- [x] T010 `GET /api/admin/comite/apelaciones/[id]/documento` (solo COMITE_VALIDACION;
  descifra + streamea; AuditLog + AccesoDocumentoApelacion; 403 admin/operador/padre;
  410 purgado) + `route.test.ts`.
- [x] T011 `POST .../[id]/tomar` (RECIBIDA→EN_REVISION, 409 si tomada) y
...
- [x] T012 `src/lib/apelacion-mantenimiento.ts` (`purgarDocumentosVencidos`,
...
- [x] T016 Enmienda constitucional (texto exacto del brief) en
  `.specify/memory/constitution.md` — commit propio de docs.
- [x] T017 `specs/README.md` (índice + contadores; lo exige `specs-discipline.test.ts`).
- [x] T018 Gate: `npx tsc --noEmit` + `npm run lint` + tests tocados + `npm run build` +
  `npm run test` completo.
- [x] T019 `cierre.md` + sección Implementación en `spec.md` (Status → IMPLEMENTADO) +
