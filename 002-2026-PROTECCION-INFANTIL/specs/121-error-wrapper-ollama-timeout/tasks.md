...
- [ ] T001 Exportar `formatZodError` desde `src/lib/validation.ts` (aditivo).
- [ ] T002 [P] Crear `src/lib/api-handler.ts`: `errorToResponse` (AppError →
  status/toJSON; ZodError → 400 VALIDATION_ERROR; resto → console.error + 500
  genérico sin filtrar `code` ni `message`) + `withErrorHandler`.
- [ ] T003 [P] Crear `src/lib/api-handler.test.ts`: equivalencia con lógica
...
- [ ] T004 Verificar T003 verde bajo candado y commitear pieza central sola.

## Fase 2 — Timeout de Ollama (commit propio)

- [ ] T005 `src/lib/ai/ollama-config.ts`: `getOllamaTimeoutMs()` (param
  `ia.ollama.timeout_ms`, entero > 0, default 120 000 ms, fallback silencioso).
- [ ] T006 `src/lib/ai/ollama-client.ts`: `AbortSignal.timeout(await
  getOllamaTimeoutMs())` en los dos fetch a `/api/generate`. Sin más cambios.
- [ ] T007 [P] `src/lib/ai/ollama-timeout.test.ts`: efecto del parámetro sobre
  el timeout aplicado; fetch colgado aborta; param inválido → default.
- [ ] T008 `prisma/seed.ts`: upsert aditivo `ia.ollama.timeout_ms` = `"120000"`
  (INTEGER, SYSTEM, no público) con descripción.
- [ ] T009 Verificar T007 verde bajo candado y commitear (config + client +
  test + seed).

## Fase 3 — Migración de rutas (commits por zona)

- [ ] T010 Zona colegio/cursos (4 archivos): `colegio/cursos/route.ts`,
  `colegio/cursos/[id]/route.ts`, `colegio/cursos/[id]/estado/route.ts`,
  `colegio/cursos/[id]/alumnos/route.ts`. Tests de la zona verdes.
- [ ] T011 Zona colegio/alumnos (3 archivos): `colegio/alumnos/[id]/route.ts`,
...
- [ ] T012 Zona colegio/alertas + identificadores (4 archivos):
...
- [ ] T013 Zona admin/colegios (2 archivos): `admin/colegios/route.ts`,
  `admin/colegios/[id]/route.ts`. Tests de la zona verdes.
- [ ] T014 Zona admin/operadores (2 archivos): `admin/operadores/route.ts`,
  `admin/operadores/[id]/route.ts`. Tests de la zona verdes.
- [ ] T015 Zona admin/comite/integrantes (2 archivos):
  `admin/comite/integrantes/route.ts`,
  `admin/comite/integrantes/[id]/route.ts`. Tests de la zona verdes.
- [ ] T016 Zona admin/reportes-revision (1 archivo):
  `admin/reportes-revision/[id]/reasignar/route.ts`.
- [ ] T017 Verificación global: 0 ocurrencias de
...
- [ ] T018 Gate bajo candado: `npx tsc --noEmit` + `npm run lint` + tests
  tocados + `npm run build`.
- [ ] T019 Suite completa `npm run test` bajo candado (incl.
  `efecto-motor-111.test.ts` verde: ninguna decisión del motor cambió).
- [ ] T020 `cierre.md` + sección Implementación en `spec.md` + tabla de
