...
- [ ] T001 `src/lib/dal/repositories/colegio-resumen.ts` — `homeRector(colegioId)`
...
- [ ] T002 [P] Extensiones de repos (sin cambiar semántica existente):
...
- [ ] T003 [P] `src/lib/colegio/semaforo.ts` (`resolverEstado`, pura: rubí = nuevas >
  0, ámbar = 72 h > 0) + test · `src/lib/colegio/fechas-humano.ts` (fecha larga ES +
  relativo "hace 12 minutos") + test
- [ ] T004 [P] `src/lib/worker-heartbeat.ts` (`leerHeartbeatWorker()` lee
