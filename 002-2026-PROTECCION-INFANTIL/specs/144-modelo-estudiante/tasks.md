...
- [ ] T001 Renombrar en `prisma/schema.prisma`: `model Alumno → Estudiante` con
...
- [ ] T002 `model IdentificadorAlumno → IdentificadorEstudiante` con
...
- [ ] T003 Campos nuevos en `Estudiante`: `apellidos String @default("")`,
  `documentoTipo String?`, `documentoNumero String?`; modelo nuevo
  `AcudienteEstudiante` (orden 1|2, `@@unique([estudianteId, orden])`,
  `@@index([estudianteId])`)
- [ ] T004 Generar migración aditiva (`prisma migrate dev`) y VERIFICAR el SQL: solo
...
- [ ] T005 Test de humo del backfill: existentes con `apellidos = ''`, resto NULL;
  re-deploy = no-op (quickstart §3)

## Fase 2 — Cascada DAL y lib (US1)

- [ ] T006 [P] Renombrar `src/lib/dal/repositories/alumno.ts → estudiante.ts`
...
- [ ] T007 [P] Renombrar `identificador-alumno.ts → identificador-estudiante.ts` +
  ...
  fortalecidas)
- [ ] T008 [P] `src/lib/colegio/alertas.ts`, `src/lib/colegio/patrones.ts`,
  `src/lib/colegio/permisos.ts`: relaciones/tipos renombrados, cero cambio de lógica
- [ ] T009 [P] Test utils: `src/lib/reporte-test-utils.ts`, `src/lib/test-utils.ts`,
  `src/lib/e2e/journeys/*` (factories crean con `apellidos`)

## Fase 3 — Validación de alta y carga (US3)

- [ ] T010 `src/lib/schemas`: `alumnoBodySchema → estudianteBodySchema` con
...
- [ ] T011 `POST /api/colegio/cursos/[id]/alumnos`: exige `apellidos` (400 humano),
...
- [ ] T012 Carga masiva (`src/lib/colegio/carga/{parser,validator,importer,
...
- [ ] T013 Tests A/B tenant (dos colegios) en cada verbo tocado: GET/POST
