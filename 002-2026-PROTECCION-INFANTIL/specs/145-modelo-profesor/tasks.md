...
- [x] T001 `prisma/schema.prisma`: modelo `Profesor` (nombre, apellidos, email?,
...
- [x] T002 Migración aditiva (`migrate diff` + shadow DB, TTY no disponible) —
...
- [x] T003 [P] `src/lib/dal/repositories/profesor.ts` (tenant-first: listar paginado
...
- [x] T004 [P] `profesorBodySchema` + `profesorPatchSchema` en `src/lib/schemas`
  (nombre+apellidos requeridos en create, email formato, estado ∈ activo|inactivo)
  + tests de schema
- [x] T005 [P] `curso.ts` (repo) y endpoints de curso: `profesorTitularId?` en
  create/update con validación same-tenant (`null` desasigna) — D1=A

## Fase 3 — Rutas CRUD (US2)

- [x] T006 `GET/POST /api/colegio/profesores` (patrón de
...
- [x] T007 `GET/PATCH /api/colegio/profesores/[id]` + `route.test.ts` A/B: 404
  cross-tenant en ambos verbos, edición, **baja suave conserva fila**, audit con
  acciones nuevas
- [x] T008 Tests de asignación (COND-1/COND-2): **negativo cross-tenant** (profesor
...
- [x] T009 [P] O-2: `src/components/ui/LuzAmbiental.test.tsx` (render por estado,
  token aplicado, aria-hidden, reduced-motion — patrón `PanelVidrio.test.tsx`)
- [x] T010 [P] O-1: fixture `rubrica:M1+M2` + votos `modelo: "M2"` y aserciones
  `not.toContain("M1")`/`not.toContain("M2")` amplias en
  `mis-reportes/[id]/route.test.ts`
- [x] T011 Regenerar `docs/architecture/01-modelo-datos.md` (+ oráculo 51→52 si
  aplica) + `arch:check` VERDE + `tokens:check` sin subir del piso (1166)
- [x] T012 Quickstart completo + gate (tsc && lint && tokens:check && test:coverage
