...
- [x] T001 Schema: `PreferenciaAlertaColegio` + `RegistroAvisoColegio` +
      `AccionAudit` ×2 + migración aditiva (diff+shadow, **I-49: cero DROP INDEX**)
      + `migrate reset && deploy && seed` en test
- [x] T002 [P] Repos `preferencia-alerta-colegio.ts` (upsert por tipo, tenant) y
...
- [x] T003 `src/lib/colegio/avisos.ts` — pipeline: registrar evento → preferencia →
      ...
      ventana móvil)
- [x] T004 `email.ts` extendido (4 funciones por tipo, copy ciego §3) + hook en
      ...
      Resend mockeado
- [x] T005 `GET/PATCH /api/colegio/preferencias-avisos` (upsert, Zod, audit) +
      página `/dashboard/colegio/configuracion` + nav "Configuración" + tests A/B
- [x] T006 Seeds `colegio.notificaciones.*` + `colegio.avisos.*` + regenerar
