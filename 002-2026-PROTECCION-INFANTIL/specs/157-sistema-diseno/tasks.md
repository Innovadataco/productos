...
- [ ] T001 Variables CSS en `src/app/globals.css` (`:root`/`.dark`): color por canal
...
- [ ] T002 `tailwind.config.ts`: colores por `rgb(var(--…-rgb) / <alpha-value>)`,
...
- [ ] T003 Reescritura de la capa semántica sobre tokens (`glass*`, `text-body`,
...
- [ ] T004 Descargar woff2 de fonts.gstatic.com (UA navegador contra css2): Instrument
...
- [ ] T005 `src/app/layout.tsx`: `next/font/local` para las 3 familias
  (`--font-instrument-sans`, `--font-instrument-serif`, `--font-dm-mono`); Inter
  FUERA (import y variable); `themeColor` → pino (`#0b6e5a`)
- [ ] T006 Escala tipográfica §4.1 como utilidades en `globals.css` (titular de
...
- [ ] T007 [P] `src/components/ui/Anillo.tsx` + test: dos anillos SVG concéntricos
...
- [ ] T008 [P] `LuzAmbiental.tsx` + `PanelVidrio.tsx` + tests: campo de luz por
  estado (pino/ámbar/rubí); panel `backdrop-filter: saturate(185%) blur(22px)`, luz
  interior en oscuro
- [ ] T009 [P] `Declaracion.tsx` + test: titular serif con palabra de estado en
  cursiva serif y color de estado (token)

## Fase 4 — Movimiento, ratchet y cierre

- [ ] T010 Keyframes del sistema con la curva única (entradas escalonadas, dibujo,
  pulso 3,4 s como ÚNICO bucle) + media query global `prefers-reduced-motion` que
  apaga TODO (research D-R4)
- [ ] T011 `scripts/tokens-check.ts`: cuenta color crudo en `src/**` (excluye
...
- [ ] T012 Contraste de pares de tokens ≥ 4.5:1 en ambos temas (script a11y o
  verificación en test del sistema)
- [ ] T013 Verificación: conteos semánticos ≥ base (109/457/375/165) ·
...
- [ ] T014 Gate (tsc && lint && tokens:check && test:coverage && build &&
