...
- [x] T001 Schema: `SeguimientoCaso` + `NotaSeguimiento` + `AccionAudit`
      `COLEGIO_CASO_NOTA_AGREGADA` + migración aditiva (diff+shadow, **I-49: cero
      DROP INDEX**) + reset/deploy/seed en test
- [x] T002 [P] Repo `seguimiento-caso.ts` (obtenerOCrearPorAlerta, agregarNota —
      ...
      tests A/B
- [x] T003 `src/lib/colegio/seguimiento.ts` — `armarTimeline()` (hitos con fuentes
      reales, pendientes honestos) + `calcularPendientes()` (puro) + tests
- [x] T004 `GET /api/colegio/alertas/[id]` (UNA llamada, 404 ajeno) + `POST
      .../notas` (withUnitOfWork, Zod 1..1000, audit) + PATCH/DELETE notas → 404 +
      route.test.ts A/B + atomicidad
- [x] T005 [P] Componentes `TimelineCaso` + `PendientesCaso` + `BitacoraCaso` +
      página `[id]` + enlace desde la lista de alertas + tests
- [x] T006 Arch: regenerar 01/02/03 + oráculos (modelos 54→56, páginas 56→57) +
      `arch:check` VERDE
