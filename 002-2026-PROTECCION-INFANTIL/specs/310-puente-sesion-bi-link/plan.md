...
4. **Claim `role` singular string (corrección bilateral 2026-08-29 21:25 COT)**: el payload efímero emite `role: user.rol` (string, clave en inglés, sin transformar) — NO `roles` en arreglo. Confirmado por Fábrica PI-1 + Fábrica BI-2 tras auditar `src/lib/auth/sesion.ts:27` del producto BI, que parsea `payload.role` como string. El modelo de roles de PI sigue siendo single-rol; el claim solo refleja eso tal cual, sin envolver en arreglo.
