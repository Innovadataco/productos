...
> **✅ Migración C — estrategia cerrada por el CEO (2026-08-30): placeholder por-fila, sin truncate.** Postgres no admite `NOT NULL` sin default sobre tabla con filas, y `IdentificadorProfesor` es referenciado por `AlertaColegio.identificadorProfesorId` (schema:1391) → un truncate arrastra alertas, descartado. La migración: agregar columnas nullable → `UPDATE` las 2 filas vivas con placeholders **por-fila y únicos que GRITAN** (`numeroDocumento='MIGR-'||id`, `email='migracion-'||id||'@placeholder.invalid'`, `telefono='MIGR-000'`, `anioNacimiento=0`, `tipoDocumento='OTRO'`) → `SET NOT NULL` + UNIQUE. **NO** es el temp-default constante (chocaba con el unique). La migración lleva un **bloque de comentario** que explica el porqué de esas filas (2 de prueba, placeholder por el NOT NULL, se las lleva el `reset-piloto` post-deploy). Los 2 profesores de prueba sobreviven editables.
