...
**✅ DESBLOQUEADA** (CEO, 2026-08-30 19:52): estrategia = **placeholder por-fila autocontenido, sin truncate**. Truncate descartado (arrastra `AlertaColegio` vía FK `identificadorProfesorId`, schema:1391). Dos condiciones no negociables del CEO en T028.
...
- [ ] T028 [US2] Migración C (identidad profesor), **autocontenida sin truncate**: (a) agregar columnas nullable → (b) `UPDATE` las 2 filas vivas con placeholders **por-fila y únicos** → (c) `SET NOT NULL` + `@@unique([colegioId, tipoDocumento, numeroDocumento])`. Verificar estado de `Profesor` antes; reportar señal pre-migración.
  - **Condición CEO 1 — los placeholders GRITAN** (nadie los confunde con dato real; Jelkin puede abrir esas fichas): `numeroDocumento='MIGR-'||id`, `email='migracion-'||id||'@placeholder.invalid'`, `telefono='MIGR-000'`, `anioNacimiento=0` (sentinela inválido), `tipoDocumento='OTRO'`. Para `sexo`: si el tipo no admite un valor que grite, alcanza con que documento/email marquen la fila — **dejarlo anotado en el comentario** si una columna no puede gritar por valor.
  - **Condición CEO 2 — la migración se auto-documenta**: bloque de comentario en el archivo explicando que son 2 filas de prueba pre-existentes, placeholder por el `NOT NULL`, y que el `reset-piloto` post-deploy se las lleva. Que nadie lea el `UPDATE` sin contexto en dos semanas.
