...
### 4) `tokens-check.ts` — piso intacto (1021)Los 16 crudos salen; con SPEC-466 (`<=`) el piso **no** se toca (medido sobre `origin/main` fresco, no sobre la rama pre-rebase).
...
Un componente UI migrado, una clase utility nueva en `globals.css`, un test y su candado. Sin schema, sin API, sin runtime.

## Cómo se probó

- `Alerta.test.tsx` — 6 tests verdes (tokens por tono, rubi solo error, cero crudos, icono, role status).
- `npm run tokens:check` — conteo < piso 1021, VERDE.
