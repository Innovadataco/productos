...
| [429](429-encuestas-y-cruce/spec.md) | SPEC-429 (A-75 · brief §7 L6-bis + §9-bis · orden CEO 23:5x) · Las dos encuestas y el cruce — cinco preguntas de opción por lado (sin estrellas ni texto libre) que se activan al `CUMPLIDA`. **Cruce automático** de r1 (¿se dio?) y r2 (¿a tiempo?): si contradice → `IncidenteContradiccionEncuesta` al Verificador. **Guardia nueva `encuestaPendiente`** estilo `debeCambiarPassword` — quien no responde queda encerrado salvo cambiar contraseña. Retira el modelo huérfano `EncuestaPrimeraCita` (SPEC-388a). Contrato de unión con SPEC-427: `al-cumplir.ts`. | 🟡 En desarrollo |
...
| [420](420-borrado-por-lotes/spec.md) | SPEC-420 · El borrado por lotes — el borrado de lo sembrado murió en producción con «too many bind variables... expected maximum of 32767, received 37176»: un `in:` gasta un parámetro por id. **La corrida de ensayo escribió 30.254 y pasó: una base de prueba más chica que producción no prueba el límite.** Tandas de 2.000 en todo el camino de borrado (incluidos los `count` del dry-run), `notIn` reemplazado por LEFT JOIN, y convención verificada por candado. Probado a 40.000 y 80.000. | 🟢 Implementada |
