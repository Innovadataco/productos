...
| [400](400-guardianes-cerrados/spec.md) | SPEC-400 (PR 1 de 2 · cierra parcialmente I-236) · Cliente resiliente para el refresco silencioso de la cookie `sesion_estado` — interceptor global sobre `globalThis.fetch` que atrapa `401 { code: SESION_ESTADO_REQUERIDO }`, dispara `POST /api/vigencia/refresh` con single-flight y reintenta el request una vez. Sin cambios en servidor: el cerrojo del middleware llega en SPEC-400b después de verificar PR 1 en producción. | 🟡 En desarrollo |
