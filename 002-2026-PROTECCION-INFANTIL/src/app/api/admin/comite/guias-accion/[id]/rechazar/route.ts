...
import { guiaAccionIdParamsSchema, guiaAccionRechazarBodySchema } from "@/lib/schemas/guia-accion";
...
export async function POST(request: Request, context: RouteContext) {
    try {
        ...
        const parsed = guiaAccionRechazarBodySchema.safeParse(body);
        if (!parsed.success) {
            return NextResponse.json(
        ...
        const guia = await new GuiaAccionService().rechazar(paramsParsed.data.id, {
            usuarioId: user.id,
            email: user.email,
            nombre: user.nombre,
        ...
        return errorToResponse(error, "[ADMIN/COMITE/GUIAS-ACCION/RECHAZAR]");
    }
}
