...
describe("GET /api/admin/ia/simulaciones/[id]/analisis", () => {
    beforeEach(async () => {
        await resetDatabase();
    });
    ...
    it("returns aggregated metrics", async () => {
        ...
        const req = crearRequestAutenticado("GET", `http://localhost/api/admin/ia/simulaciones/${run.id}/analisis`, null);
        const res = await GET(req, { params: Promise.resolve({ id: run.id }) });
        expect(res.status).toBe(200);
        const body = await res.json();
        expect(body.metricas.accuracy).toBe(1);
        expect(body.metricas.latenciaP50Ms).toBe(120);
    });
});
