...
describe("GET /api/admin/ia/simulaciones/[id]/resultados", () => {
    beforeEach(async () => {
        await resetDatabase();
    });

    it("returns per-case results with accuracy flag", async () => {
        ...
        const req = crearRequestAutenticado("GET", `http://localhost/api/admin/ia/simulaciones/${run.id}/resultados`, null);
        const res = await GET(req, { params: Promise.resolve({ id: run.id }) });
        expect(res.status).toBe(200);
        const body = await res.json();
        expect(body.items).toHaveLength(1);
        expect(body.items[0].acierto).toBe(true);
        expect(body.items[0].categoriaAsignada).toBe("SOLICITUD_ENCUENTRO");
    });
});
