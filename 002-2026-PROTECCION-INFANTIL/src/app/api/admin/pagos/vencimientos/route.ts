...
import { pagosVencimientosQuerySchema } from "@/lib/schemas/pagos";
...
export async function GET(request: Request) {
    try {
        ...
        const parsed = pagosVencimientosQuerySchema.safeParse(Object.fromEntries(url.searchParams.entries()));
        if (!parsed.success) {
            return NextResponse.json(
        ...
        const { page, pageSize, dias } = parsed.data;
        ...
        return errorToResponse(error, "[ADMIN/PAGOS/VENCIMIENTOS]");
    }
}
