/**
 * SPEC-291 (002-PI-191) — POST /api/admin/servicios/<nombre>/restart
 * Requiere permiso sistema_admin + header X-Confirm-Action: yes.
 * Audit: LOGS_MANTENIMIENTO_PURGA + metadatos.tipo="servicio_restart".
 */
...
export async function POST(request: Request, { params }: { params: Promise<{ nombre: string }> }) {
    return handlerAccionServicio(request, "restart", params);
}
