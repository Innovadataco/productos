...
import { comiteCuentaSchema } from "@/lib/schemas";
...
export async function GET(request: Request) {
    try {
        ...
        const cuenta = await new ComiteConvivenciaService().obtenerCuenta(user.colegioId);
        return NextResponse.json({ cuenta });
    } catch (error) {
        return errorToResponse(error, "[COLEGIO/COMITE/CUENTA]");
    }
}
...
export async function POST(request: Request) {
    try {
        ...
        const body = await withValidation.body(comiteCuentaSchema)(request);
        const resultado = await new ComiteConvivenciaService().crearCuenta(
            user.colegioId,
            body.email,
            user.id,
