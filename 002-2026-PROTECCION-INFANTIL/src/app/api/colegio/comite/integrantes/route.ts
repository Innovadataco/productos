...
import { integranteComiteConvivenciaSchema } from "@/lib/schemas";
...
export async function GET(request: Request) {
    try {
        ...
        const integrantes = await new ComiteConvivenciaIntegrantesService().listar(user.colegioId);
        return NextResponse.json({ integrantes });
    } catch (error) {
        return errorToResponse(error, "[COLEGIO/COMITE/INTEGRANTES]");
    }
}
...
export async function POST(request: Request) {
    try {
        ...
        const body = await withValidation.body(integranteComiteConvivenciaSchema)(request);
        const integrante = await new ComiteConvivenciaIntegrantesService().crear(
            user.colegioId,
            body,
            user.id,
