...
import { cuidIdSchema, resolverSolicitudSchema } from "@/lib/schemas";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const body = await withValidation.body(resolverSolicitudSchema)(request);

        const resultado = await new ComiteConvivenciaBandejaService().resolver(
            user.comiteColegioId,
            id,
            body,
            user.id,
        ...
        return errorToResponse(error, "[COLEGIO/COMITE/SOLICITUDES/[ID]/RESOLVER]");
    }
}
