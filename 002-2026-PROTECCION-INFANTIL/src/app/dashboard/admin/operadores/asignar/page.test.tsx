 ...
 * esperar el cron cada 15 min. Los tests atan las tres reglas que no pueden
 * cambiar sin darse cuenta:
 *   · deshabilitado si la cola ya está en 0 (no hay nada que asignar);
 *   · dispara `POST /api/admin/operadores/reconciliacion` y muestra el resumen;
 *   · después del disparo, refresca el resumen de la cola.
 */
...
const ESTADO_LIMPIO = { ...ESTADO_CON_COLA, sinAsignar: 0 };
...
describe("Botón 'Asignar huérfanos ahora' (SPEC-372 · A-74 · P3)", () => {
    ...
    it("se DESHABILITA cuando no hay reportes sin asignar (nada que hacer)", async () => {
        mockFetchRuteado({ "/api/admin/operadores/asignacion": ESTADO_LIMPIO });
        render(<AdminOperadoresAsignarPage />);
        await waitFor(() => expect(screen.getByText(/Casos sin asignar/)).toBeTruthy());
        const boton = screen.getByRole("button", { name: /Asignar huérfanos ahora/ }) as HTMLButtonElement;
        expect(boton.disabled).toBe(true);
    });

    it("con cola > 0 dispara POST /reconciliacion, muestra el resumen y refresca la cola", async () => {
        let llamadasEstado = 0;
        let llamadasReconciliacion = 0;
        mockFetchRuteado({
            "GET /api/admin/operadores/asignacion": () => {
                // Antes del disparo: 3 en cola. Después del disparo: 1 (2 asignados).
                llamadasEstado += 1;
                return llamadasEstado === 1 ? ESTADO_CON_COLA : { ...ESTADO_CON_COLA, sinAsignar: 1 };
            },
            "POST /api/admin/operadores/reconciliacion": () => {
        ...
        await waitFor(() => {
            expect(llamadasReconciliacion).toBe(1);
        });
        await waitFor(() => {
            // Resumen visible con los tres números en el aviso.
            const status = screen.getByRole("status");
            expect(status.textContent).toContain("3");
            expect(status.textContent).toContain("2");
            expect(status.textContent).toContain("1");
        });
        // Y refrescó la cola: dos GETs. El "1" resultante convive con el "1" de
        // "Operadores activos" en el DOM, así que verificamos por número de
        // llamadas (fuente única de verdad) en vez de por texto ambiguo.
        expect(llamadasEstado).toBe(2);
    });

    it("si el endpoint devuelve 403, muestra el mensaje del servidor sin romper la cola", async () => {
        vi.spyOn(global, "fetch").mockImplementation(async (input, init) => {
            ...
            if (method === "POST" && url.includes("/api/admin/operadores/reconciliacion")) {
                return {
                    ok: false,
                    status: 403,
        ...
        await waitFor(() => {
            expect(screen.getByRole("status").textContent).toContain("No autorizado");
        });
