...
/**
 * SPEC-289 (002-PI-189 · Fase 1): descuento de bono en COP puro.
 *
 * Análogo a `calcularDescuentoBono` (USD) pero con base COP como fuente única.
 * Regla:
 *  - DESCUENTO_PCT: `pct × precioBaseCOP` (cero tasa).
 *  - MESES_GRATIS: `valor × precioBaseCOP` mensual (cero tasa).
 *  - DESCUENTO_FIJO_USD: `valor × tasaFallback` (fallback 1:1 si no hay tasa).
 *    Documentado: la Fase 2 (ARQ_16) migrará estos bonos a `descuentoCOP` real
 *    en la BD; hasta entonces la conversión al vuelo es la salida menos mala
 *    (no lanza 503; UX consistente con el resto del checkout).
 *
 * Redondeo a cents (COP entero). Nunca negativo. Nunca supera `precioBaseCOP`.
 */
export function calcularDescuentoBonoCOP(
    precioBaseCOP: number,
    bono: BonoCalculable,
    tasaFallback: number = 1,
): number {
    if (!Number.isFinite(precioBaseCOP) || precioBaseCOP <= 0) return 0;
    if (!Number.isFinite(bono.valor) || bono.valor < 0) return 0;
    ...
    switch (bono.tipo) {
        case "DESCUENTO_PCT": {
            const pct = Math.min(bono.valor, 100);
            descuento = (precioBaseCOP * pct) / 100;
            break;
        }
        case "DESCUENTO_FIJO_USD": {
            const tasa = Number.isFinite(tasaFallback) && tasaFallback > 0 ? tasaFallback : 1;
            descuento = bono.valor * tasa;
            break;
        }
        case "MESES_GRATIS":
            descuento = bono.valor * precioBaseCOP;
            break;
        default:
            descuento = 0;
    }
    return Math.max(0, Math.min(Math.round(descuento), precioBaseCOP));
}
