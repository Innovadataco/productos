...
export async function resetDatabase(tablas?: string[]): Promise<void> {
    // SPEC-344 (hallazgo 01-09-2026): GUARD DURO — este reset TRUNCA la base
    // que apunte DATABASE_URL, sea cual sea. La BD dev compartida de esta Mac
    // amaneció arrasada (0 colegios, 0 parámetros, huella de plataformas del
    // propio reset) por un proceso que corrió tests con el env equivocado.
    // Regla: solo se trunca una base cuyo nombre contenga "test". Cualquier
    // otra cosa es un error de configuración y se aborta EN VOZ ALTA.
