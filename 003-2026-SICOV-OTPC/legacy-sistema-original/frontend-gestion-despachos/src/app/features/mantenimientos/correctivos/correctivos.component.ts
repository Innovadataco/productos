...
import { RegistroCorrectivoComponent } from './registro-correctivo.component';
import { of } from 'rxjs';
import { catchError, finalize } from 'rxjs/operators';
import { Rol } from 'src/app/core/models/auth.models';
import { ParametricasService, RawTipoIdentificacion, TipoIdentificacion } from 'src/app/parametricas/servicios/parametricas.service';
...
@Component({
  selector: 'app-correctivos-view',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
    ModalComponent,
    RegistroCorrectivoComponent,
    PaginatorComponent,
    DatePipe,
  ],
    ...
    <div class="d-flex flex-column gap-3">
      <div class="card border-1 shadow-sm" *ngIf="rol?.id === 2 || rol?.id === 1">
        <div class="card-body d-grid gap-3">
          ...
          >
            <h5 class="card-title mb-0">PDF del programa de mantenimiento correctivo</h5>
              ...
              <tbody>
                <tr *ngFor="let d of documentosPaginados()">
                  ...
                  <td>
                    <span
                      class="badge"
                      [class.text-bg-success]="d.estado"
                      [class.text-bg-secondary]="d.estado === false"
                    >
          ...
          <app-paginator
            ...
            (pageSizeChange)="docsPageSize = $event; docsPage = 1"
            [maxButtons]="7"
            [storageKey]="'correctivos_docs'"
          ></app-paginator>
          }
        </div>
      </div>
      ...
      <div class="card border-1 shadow-sm" *ngIf="rol?.id === 3 || rol?.id === 1">
        <div class="card-body d-grid gap-3">
          ...
          <div class="table-responsive border rounded">
            <table class="table align-middle mb-0">
              ...
              <tbody>
                ...
                <tr *ngIf="!registrosLoading && registrosError">
                  <td colspan="5" class="text-center text-danger py-4">{{ registrosError }}</td>
                </tr>
                <!-- <tr *ngIf="!documentos.length">
                  <td colspan="5" class="text-center text-muted py-4">Debe cargar el programa de mantenimiento correctivo.</td>
                </tr> -->
                <tr *ngIf="documentos.length === 0 && !registrosError">
                  <td
                    *ngIf="!registrosLoading && !registros.length"
                    colspan="5"
                  ...
                  <td
                    *ngIf="!registrosLoading && registros.length && !registrosPaginados().length"
                    colspan="5"
                    class="text-center text-muted py-4"
                  >
                    Sin resultados
          ...
          @if (registrosFiltrados().length > pageSize) {
          <app-paginator
            ...
            [maxButtons]="7"
            [storageKey]="'correctivos_registros'"
          ></app-paginator>
          }
        </div>
      </div>
      ...
      <app-modal
        [open]="historialOpen()"
        [title]="'Historial de mantenimiento correctivo' + ' - ' + placaFiltro"
        size="xl"
        (closed)="historialOpen.set(false)"
      >
      <div class="d-flex align-items-center gap-2 mb-3 justify-content-end">
        <div class="input-group input-group-sm" style="max-width: 240px;">
          <label for="historialBuscar" class="visually-hidden">Buscar</label>
          <input id="historialBuscar" type="search" class="form-control form-control-sm" placeholder="Buscar..."
          ...
          <table class="table align-middle mb-0">
            ...
            <tbody>
              ...
              <tr *ngFor="let r of pagedHistorial(); let i = index">
                ...
                <td>
                  <div class="fw-semibold">{{ r.nombres_responsable || r.nombreIngeniero }}</div>
                  <div class="text-muted small" *ngIf="getIdentText(r.tipo_identificacion || r.tipoIdentificacion, r.numero_identificacion || r.numeroIdentificacion)">
                    {{ getIdentText(r.tipo_identificacion || r.tipoIdentificacion, r.numero_identificacion || r.numeroIdentificacion) }}
                  </div>
                </td>
                <td style="max-width: 300px; white-space: pre-wrap;">
                  {{ r.detalle_actividades || r.detalleActividades }}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div class="mt-2">
          <app-paginator
            ...
            (pageSizeChange)="pageSize = $event; page = 1"
            [maxButtons]="7"
            [storageKey]="'correctivos_historial'"
          ></app-paginator>
        </div>
        }
      </app-modal>
      ...
      <app-modal
        ...
        [title]="
          placaSeleccionada
            ? 'Nuevo mantenimiento correctivo — Placa: ' + placaSeleccionada
            : 'Nuevo mantenimiento correctivo'
        "
        size="xl"
        (closed)="isRegistroOpen.set(false)"
      >
        <app-registro-correctivo
          [inModal]="true"
          [initialPlaca]="placaSeleccionada"
          [resetToken]="registroReset"
          [modalOpen]="isRegistroOpen()"
...
export class CorrectivosComponent {
  ...
  private readonly TIPO_CORRECTIVO = 2;
  placaFiltro = '';
  placaSeleccionada: string | undefined;
  historial: any[] = [];
  // Lista base sin filtros para combinar término y fecha
  historialBase: any[] = [];
  page = 1;
  pageSize = 5;
  ...
  filtro = '';
  pageSizeVeh = 7; // same as pageSize for simplicity
  filtroDocs = '';
  terminoHistorial = '';
  fechaHistorial = '';
  ...
  docsPage = 1;
  docsPageSize = 5;
  // Token para reiniciar el formulario hijo al abrir el modal
  registroReset = 0;
  ...
  documentosPaginaAnterior() {
    if (this.docsPage > 1) this.docsPage--;
  }
  documentosPaginaSiguiente() {
    if (this.docsPage < this.documentosTotalPaginas()) this.docsPage++;
  }
  documentosRango() {
  ...
  private cargarLista() {
    ...
    this.api
      .listarDocumentos(this.TIPO_CORRECTIVO, vigilado)
      .pipe(
        catchError((err) => {
          if (err instanceof HttpErrorResponse && err.status === 401) {
            this.documentosError = (err.error?.mensaje ?? err.message ?? 'Acceso no autorizado').toString();
          } else {
      ...
      .subscribe((res: any) => {
        ...
        this.documentosError = null;
        this.documentos = lista.length ? lista : [];
      });
  }
  ...
  onFileChange(evt: Event) {
    ...
    this.archivos.guardarArchivo(file, 'sicov', vigilado).subscribe({
      next: (resp) => {
        this.api
          .guardarArchivo(
            this.TIPO_CORRECTIVO,
            resp.nombreAlmacenado,
            resp.nombreOriginalArchivo,
            resp.ruta,
            vigilado
          )
          .subscribe({
  ...
  mostrarInfo() {
    this.api.descripcionPlantillaMasivo('correctivo');
  }

  descargarPlantillaMasivo() {
    const url = '/api/v1/mantenimiento/plantillas/preventivo-correctivo'
    const nombreArchivo = 'plantilla_mantenimiento_correctivo.xlsx';
  ...
  onFileChangeMasivo(evt: Event) {
    ...
    this.api.cargueMasivo(file, 'correctivo').subscribe({
      next: (res) => {
      ...
      error: (error) => {
        ...
        Swal.fire({
          ...
          if (res.isConfirmed) {
            this.descargarErroresTxt(erroresTexto, 'errores_cargue_correctivo.txt');
          }
        });
        input.value = '';
      },
  ...
  cargarHistorial() {
    ...
    this.api
      .historial(this.TIPO_CORRECTIVO, vigilado, placa)
      .pipe(
        catchError(() => of([])),
        finalize(() => this.cdr.markForCheck())
      )
  ...
  exportarHistorial(): string | null {
    ...
    return this.api.exportarHistorial(this.TIPO_CORRECTIVO, vigilado, placa) as string;
  }
  ...
  protected getIdentText(tipo: unknown, numero: unknown): string {
  ...
  private cargarRegistros() {
    ...
    this.api
      .listarRegistros(this.TIPO_CORRECTIVO, vigilado, rolId)
      .pipe(
        finalize(() => {
  ...
  totalPaginas() {
    const t = this.registrosFiltrados().length;
    return t ? Math.max(1, Math.ceil(t / this.pageSize)) : 1;
  }
  paginaAnterior() {
    if (this.page > 1) this.page--;
  }
  ...
  filtrarHistorial() {
    ...
    if (term) {
      lista = lista.filter((r) => {
        ...
        return (
          ...
          nombreIngeniero.includes(term) ||
          detalleActividades.includes(term)
        );
      });
    }
    ...
    // Reset page immediately and again in a microtask to avoid persisted page restoring
    this.page = 1;
    this.cdr.markForCheck();
    setTimeout(() => {
      this.page = 1;
