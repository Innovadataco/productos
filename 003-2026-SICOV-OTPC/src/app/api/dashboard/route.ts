...
export async function GET() {
  try {
    ...
    const [despachosHoy, enCola, procesados, fallidos, llegadasHoy, llegadasEnCola] =
      await Promise.all([
        ...
        prisma.llegadaSolicitud.count({
          where: { ...where, fechaCreacion: { gte: inicioDiaBogota() } },
        }),
        prisma.llegadaSolicitud.count({ where: { ...where, estado: "pendiente" } }),
      ]);
    ...
    return NextResponse.json({
      kpis: { despachosHoy, enCola, procesados, fallidos, nivelExito, llegadasHoy, llegadasEnCola },
    });
  } catch (err: unknown) {
