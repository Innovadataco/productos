...
/// POST /api/mantenimientos/preventivo — detalle (paso 2). Roles 1,3 (§10.2) + guard D-017.
export async function POST(req: Request) {
  try {
    const u = await verifyAuth([1, 3]);
    await requiereModulo(u, "mantenimientos", "preventivos");
    const body = (await req.json().catch(() => ({}))) as Partial<RegistroDetalle> & {
      mantenimientoId?: unknown;
    };
    const r = await guardarDetalle(1, body, u.identificacion ?? "", u.rolId ?? 0);
