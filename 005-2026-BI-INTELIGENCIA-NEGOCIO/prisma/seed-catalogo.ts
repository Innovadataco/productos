// prisma/seed-catalogo.ts · Seed idempotente del catálogo BI
// SPEC-008 · F3C 2026-08-28 · Autor: bi-dev-2
// Ejecutar: npx prisma db seed
// Regla dura: upsert({..., update: {}}) — NUNCA sobreescribir customizaciones del operador.
...
// ────────────────────────────────────────────────────────────────────────────
// Columnas por tabla (>= 80 total · campos verificados candado 15 · 2026-08-28)
// ────────────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────────────
// Seed functions (idempotentes: update: {})
// ────────────────────────────────────────────────────────────────────────────
...
async function seedTablas() {
  ...
  console.log(`  · ${TABLAS.length} tablas sembradas`);
}
...
async function seedColumnas() {
  ...
  for (const c of COLUMNAS) {
    ...
    if (!tablaId) {
      console.warn(`  ! columna ${c.tabla}.${c.nombreFuente} sin tabla padre · skip`);
      continue;
    }
  ...
  console.log(`  · ${COLUMNAS.length} columnas sembradas`);
}
...
async function seedMetricas() {
  ...
  console.log(`  · ${METRICAS.length} metricas sembradas`);
}
...
async function seedEjemplos() {
  ...
  console.log(`  · ${EJEMPLOS.length} ejemplos sembrados`);
}

async function main() {
  console.log("Seed catalogo BI · SPEC-008 · F3C 2026-08-28");
  await seedTablas();
  await seedColumnas();
  await seedMetricas();
  await seedEjemplos();
