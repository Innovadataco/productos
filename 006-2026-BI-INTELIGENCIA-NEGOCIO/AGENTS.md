...
- Drift-scan de columnas (2026-09-12 · hallazgo: `horaAproximada` faltaba en bi-db porque el shell viene del volcado del 01-09 y PI la agregó después — el hueco aparece el día que la columna entra al canon, no el día que se crea; el primer INSERT tumbaba el apply worker): `scripts/drift-scan-replica.sh` lee las columnas publicadas del MASTER y verifica cada una contra bi-db (exit 1 si falta alguna). **REGLA: corre ANTES del primer INSERT, no después del REFRESH — obligatorio tras todo cambio de canon que sume columnas a tablas ya replicadas.** No hay canal push de alarma (decisión CEO 12-09: colgarse del correo de PI sería compartir canal con el tráfico que se vigila — la alarma se apagaría sola cuando más se necesita; la bitácora admin no depende de nadie).
